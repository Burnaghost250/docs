...
def login_view(request):
    if request.user.is_authenticated:
        return redirect('boss' if request.user.is_superuser else 'dashboard')
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            login(request, user)
            return redirect('boss' if user.is_superuser else 'dashboard')
...
def delete_uploads(request, upload_id):
    # Delete upload view
    delete_url = reverse('delete_upload', args=[upload_id])
